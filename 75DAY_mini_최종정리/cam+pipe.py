...
model.classifier[3] = nn.Linear(in_features=1280, out_features=31)
# model = models.resnet50(pretrained=False)
# model.fc = nn.Linear(in_features=2048, out_features=31)

# model =  torchvision.models.swin_t(weights="IMAGENET1K_V1")
# model.head = torch.nn.Linear(in_features=768, out_features=31)
model.to(device)

model.load_state_dict(torch.load("mobile_aug_best.pt", map_location=device))
