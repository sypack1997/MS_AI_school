...
val_dataset=customDataset("./dataset/val",transform=val_trasform)
test_dataset=customDataset("./dataset/test",transform=val_trasform)
...
exp_lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.93)
...
def train(best_val_acc):
    if os.path.exists(save_path):
        best_val_acc = max(pd.read_csv("./steel_acc.csv")["Accuracy"].tolist())
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs - 1:
            df_Acc.to_csv('./steel_acc.csv', index=False)
