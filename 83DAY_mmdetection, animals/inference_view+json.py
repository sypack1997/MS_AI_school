...
# results = inference_detector(model, images) # 이걸로 확인하는거임
# show_result_pyplot(model, img, results)

"---------------------------"
# 밑에서 부터는 전체 이미지 읽어서 진행
...
for img_info in image_info['images'] :
    file_name = img_info['file_name'] # 파일 이름 읽을려고 편하게하기 위해 json 파일 읽기. 보통은 glob이용해서 각 파일 이름 가져오면 됨.
    img_height = img_info['height'] # 가져온김에 같이 가져옴
    img_width = img_info['width'] # 가져온김에 같이 가져옴
    ...
    # scale
    x_scale = float(960)/img_width # json없을때는 이미지 불러서 size확인하면 됨.
    y_scale = float(540)/img_height

    results = inference_detector(model, img_path) # inference_detector가 원래 알던 model을 output에 넣는 것과 같다. 즉, best pt를 활용하여 테스트 이미지에 결과값을 도출해줌.
    for number, result in enumerate(results) :
        if len(result) == 0 :
            continue
        # threshold setting ..
        category_id = number + 1
        result_filtered = result[np.where(result[:,4] > score_threshold)] # bounding box 좌표 값들 있음. threshold 0.5이상 값만 뽑기
        ...
        for i in range(len(result_filtered)) :
            ...
            y_max = int(result_filtered[i, 3]) # result_filtered[i,4]는 score 값임.
            ...
            tmp_dict['category_id'] = category_id # 라벨 번호 ex. 1: wine, 2: ~~, 3: ~~
            tmp_dict['area'] = json_w * json_h
            tmp_dict['image_id'] = img_info['id'] # json 없으면  results 쪽에 id +1 작성하면 될 듯
