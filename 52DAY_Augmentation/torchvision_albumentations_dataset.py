...
torchvision_transform = transforms.Compose([
    ...
    # transforms. Grayscale(num_output_channels=1),
    # transforms.ColorJitter(brightness=0.5, hue=3, cntrast=0.3),
    # transforms.GaussianBlur(kernel_size = (3,9), sigma = (0.1,5)),
...
albumentations_transform_oneof = albumentations.Compose([
    ...
    albumentations.OneOf([
        albumentations.HorizontalFlip(p=1), # p : 확률 (1 = 100%)
...
alb_dataset = alb_cat_dataset(file_paths=['cat00.jpeg'], transform=albumentations_transform)

cat_dataset = CatDataset(file_paths=["cat00.jpeg"],
                         transform=torchvision_transform)

alb_ondeof_dataset = alb_cat_dataset(file_paths=['cat00.jpeg'], transform=albumentations_transform_oneof)
...
alb_total_tiem = 0
for i in range(100):
    alb_image, alb_time = alb_ondeof_dataset[0]
    alb_total_tiem += alb_time

print("alb_oneof time >> ", alb_total_tiem*10)
...
'''
torchvision tiem/image >>  5.6156325340271
alb time >>  0.8198690414428711
'''